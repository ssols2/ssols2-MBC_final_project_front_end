    ...
    <div class="control-container">
        ...
        <div class="filter-bar">
            ...
            <div class="search-box">
                ...
                <svg class="search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" @click="executeSearch">
        ...
        <template v-if="searchActive">
            <div class="search-result-area">
                ...
                <div class="search-list-panel">
                    ...
                    <div v-for="item in searchResults" :key="item.vehicleNum" class="search-item"
                        :class="{ selected: selectedVehicle?.vehicleNum === item.vehicleNum }"
                        ...
                        <span class="si-badge" :class="item.isMember ? 'badge-member' : 'badge-guest'">{{ item.isMember
                            ? '회원' : '비회원' }}</span>
                ...
                <div class="search-detail-panel" v-if="selectedVehicle">
                    <div class="detail-images">
                        <img v-if="selectedVehicle.vehicleImg" :src="`${backendUrl}${selectedVehicle.vehicleImg}`"
                            class="detail-car-img" />
                        <div v-else class="no-detail-img">No Image</div>
                        <img v-if="selectedVehicle.licensePlateImg"
                            :src="`${backendUrl}${selectedVehicle.licensePlateImg}`" class="detail-plate-img" />
                        <div v-else class="no-detail-img">No Image</div>
                    </div>
                    <table class="detail-info-table">
                        <tbody>
                            <tr>
                                <th>국가</th>
                                <td>{{ countryName(selectedVehicle.licensePlateCountry) }}</td>
                            </tr>
                            <tr>
                                <th>회원여부</th>
                                <td>{{ selectedVehicle.memberName || (selectedVehicle.isMember ? 'Y' : 'N') }}</td>
                            </tr>
                            <tr>
                                <th>차량번호</th>
                                <td class="text-blue">{{ selectedVehicle.vehicleNum }}</td>
                            </tr>
                            <tr>
                                <th>입차시간</th>
                                <td>{{ formatDT(selectedVehicle.entryTime) }}</td>
                            </tr>
                            <tr>
                                <th>출차시간</th>
                                <td>{{ selectedVehicle.exitTime ? formatDT(selectedVehicle.exitTime) : '—' }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="table-card full-width">
                ...
                <div class="table-responsive">
                    <table class="dark-table">
                        ...
                        <tbody>
                            <tr v-if="pagedSearchHistory.length === 0">
                                <td colspan="8" class="empty-msg">데이터가 없습니다.</td>
                            </tr>
                            <tr v-for="(log, idx) in pagedSearchHistory" :key="log.parkingLogId">
                                ...
                                <td><span class="status-dot" :class="statusDotClass(log)"></span><span
                                        :class="statusTextClass(log)">{{ statusLabel(log) }}</span></td>
                                <td>{{ log.empNumber || '—' }}</td>
                ...
                <div class="pagination" v-if="searchHistoryPages > 1">
                    ...
                    <button class="page-btn" @click="goToPage(currentPage + 1)"
                        :disabled="currentPage === searchHistoryPages">&#8250;</button>
                </div>
            </div>
        </template>
                ...
                <div class="carousel-track" ref="carouselRef">
                    <div v-for="log in recentLogs" :key="log.parkingLogId" class="carousel-card">
                        ...
                        <div class="carousel-images">
                            ...
                            <img v-if="log.licensePlateImg" :src="`${backendUrl}${log.licensePlateImg}`"
                                class="plate-thumb" />
                            <div v-else class="no-thumb">No Image</div>
                        </div>
            ...
            <div class="main-row">
                <div class="table-card">
                    <div class="table-header">
                        <div class="th-left">
                            ...
                            <select v-model="countryFilter" class="dark-select">
                            ...
                            <select v-model="memberFilter" class="dark-select">
                                <option value="">회원여부</option>
                                <option value="Y">회원</option>
                                <option value="N">비회원</option>
                            </select>
                            <select v-model="parkingFilter" class="dark-select">
                                <option value="">주차여부</option>
                                <option value="parking">주차중</option>
                                <option value="exited">출차완료</option>
                            </select>
                            <select v-model="statusFilter" class="dark-select">
                                <option value="">상태</option>
                                <option value="normal">정상</option>
                        ...
                        <div class="th-right"><select v-model="sortOrder" class="dark-select plain">
                                <option value="desc">최신순</option>
                                <option value="asc">오래된순</option>
                            </select></div>
                    </div>
                    <div class="table-responsive">
                        <table class="dark-table">
                            ...
                            <tbody>
                                <tr v-if="loading">
                                    <td colspan="10" class="empty-msg">데이터 불러오는 중...</td>
                                </tr>
                                <tr v-else-if="pagedLogs.length === 0">
                                    <td colspan="10" class="empty-msg">데이터가 없습니다.</td>
                                </tr>
                                <tr v-for="log in pagedLogs" :key="log.parkingLogId">
                                    ...
                                    <td><span class="status-dot" :class="statusDotClass(log)"></span><span
                                            :class="statusTextClass(log)">{{ statusLabel(log) }}</span></td>
                                    <td>{{ log.empNumber || '—' }}</td>
                                    <td class="action-cell"><button class="action-btn edit-btn"
                                            @click="openModify(log)">수정</button><button class="action-btn del-btn"
                                            @click="deleteLog(log)">삭제</button></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="sidebar">
                    <div class="sidebar-card">
                        <h3 class="card-title">요일별 평균 체류시간</h3><canvas ref="durationChartRef"></canvas>
                    </div>
                    <div class="sidebar-card donut-wrap">
                        <h3 class="card-title">회원 / 비회원 비율</h3>
                        <div class="donut-container"><canvas ref="memberChartRef"></canvas>
                            <div class="donut-center">총 {{ allLogs.length }}명</div>
                        </div>
                        <div class="donut-legend">
                            <span class="legend-item"><span class="dot orange"></span> 회원 {{ memberCount }}명 ({{
                                memberPct }}%)</span>
                            <span class="legend-item"><span class="dot blue"></span> 비회원 {{ guestCount }}명 ({{ guestPct
                                }}%)</span>
                        </div>
                    </div>
                </div>
        ...
        <div v-if="editModal.show" class="modal-overlay" @click.self="editModal.show = false">
            <div class="modal-box">
                <div class="modal-images">
                    <img v-if="editModal.data.vehicleImg" :src="`${backendUrl}${editModal.data.vehicleImg}`"
                        class="modal-car-img" />
                    <div v-else class="modal-no-img">No Image</div>
                    <img v-if="editModal.data.licensePlateImg" :src="`${backendUrl}${editModal.data.licensePlateImg}`"
                        class="modal-plate-img" />
                    <div v-else class="modal-no-img">No Image</div>
                </div>
                <div class="modal-form">
                    <div class="form-row"><label>회원여부</label><input type="text" v-model="editModal.data.memberName"
                            class="form-input" readonly /></div>
                    <div class="form-row">
                        <label>국가</label>
                    ...
                    <div class="form-row"><label>차량번호</label><input type="text" v-model="editModal.data.vehicleNum"
                            class="form-input" /></div>
                    <div class="form-row"><label>입차일시</label><input type="datetime-local"
                            v-model="editModal.data.entryTimeLocal" class="form-input" /></div>
                    <div class="form-row"><label>출차일시</label><input type="datetime-local"
                            v-model="editModal.data.exitTimeLocal" class="form-input" /></div>
                </div>
                <div class="modal-actions">
                    <button class="modal-btn cancel" @click="editModal.show = false">취소</button>
...
const renderCharts = () => {
    ...
    if (durationChartRef.value) {
        durationChart = new Chart(durationChartRef.value, {
            type: 'bar', data: { labels: DOW, datasets: [{ data: dowAvg, backgroundColor: ['#fbb900', '#fbb900', '#fbb900', '#fbb900', '#fbb900', '#60a5fa', '#ef4444'], borderRadius: 4, barPercentage: 0.6 }] },
    ...
    if (memberChartRef.value) {
        memberChart = new Chart(memberChartRef.value, {
            type: 'doughnut', data: { labels: ['회원', '비회원'], datasets: [{ data: [memberCount.value, guestCount.value], backgroundColor: ['#fbb900', '#3b82f6'], borderWidth: 0 }] },
